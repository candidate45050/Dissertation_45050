...
def create_sensitivity_analysis(matches_df: pd.DataFrame, costplus_df: pd.DataFrame, comparison_df: pd.DataFrame) -> pd.DataFrame:
    """Create sensitivity analysis for different quantity and strength combinations"""
    sensitivity_data = []
    
    for _, row in matches_df.iterrows():
        # Get all formulations for this drug
        drug_data = costplus_df[costplus_df['drug_name_clean'] == row['costplus_drug'].upper().strip()].copy()
        
        if drug_data.empty:
            continue
            
        # Filter out rows with missing strength or quantity data
        drug_data = drug_data.dropna(subset=['strength_numeric', 'quantity_numeric'])
        
        if len(drug_data) == 0:
            continue
            
        # Get Medicare total spending from comparison dataframe to ensure consistency
        comparison_row = comparison_df[comparison_df['Cost Plus Drug'] == row['costplus_drug']]
        if not comparison_row.empty:
            medicare_total_spending = comparison_row.iloc[0]['Medicare Total Spending (2025$)']
            medicare_generic_display = comparison_row.iloc[0]['Medicare Generic Name']
        else:
            # Fallback if not found in comparison dataframe
            medicare_total_spending = row['medicare_unit_price'] * row['total_dosage_units']
            medicare_generic_display = row['medicare_generic'] if pd.notna(row['medicare_generic']) else 'N/A'
            
        total_units = row['total_dosage_units']
        
        # Find quantity and strength extremes
        min_quantity = drug_data['quantity_numeric'].min()
        max_quantity = drug_data['quantity_numeric'].max()
        min_strength = drug_data['strength_numeric'].min()
        max_strength = drug_data['strength_numeric'].max()
        
        # Scenario combinations
        scenarios = [
            ('Low Qty + Low Strength', min_quantity, min_strength),
            ('Low Qty + High Strength', min_quantity, max_strength),
            ('High Qty + Low Strength', max_quantity, min_strength),
            ('High Qty + High Strength', max_quantity, max_strength)
        ]
        
        for scenario_name, target_quantity, target_strength in scenarios:
            # Find the formulation that matches this combination
            matching_formulations = drug_data[
                (drug_data['quantity_numeric'] == target_quantity) & 
                (drug_data['strength_numeric'] == target_strength)
            ]
            
            if not matching_formulations.empty:
                # Use the first match if multiple exist
                formulation = matching_formulations.iloc[0]
                
                # Calculate total spending including fees and taxes (already in unit_price)
                unit_price_with_fees = formulation['unit_price']  # Already includes ($price + 5) * 1.0752 / quantity
                total_spending = unit_price_with_fees * total_units
                
                # Calculate savings vs Medicare
                if medicare_total_spending > 0:
                    savings_amount = medicare_total_spending - total_spending
                    savings_percentage = (savings_amount / medicare_total_spending) * 100
                else:
                    savings_amount = 0
                ...
                sensitivity_data.append({
                    ...
                    'Strength (mg)': target_strength,
                    'Base Price ($)': formulation['price'],
                    'Unit Price with Fees ($)': unit_price_with_fees,
                    'Formulation Details': f"{formulation['strength']} - {formulation['quantity']}",
                    'Total Dosage Units': total_units,
                    'Total Spending ($)': total_spending,
                    'Medicare Total Spending ($)': medicare_total_spending,
...
def main():
    ...
    with st.spinner("Matching drugs between datasets..."):
        ...
        sensitivity_df = create_sensitivity_analysis(matches_df, costplus_df, comparison_df)
    ...
    # Create tabs
    tab1, tab2, tab3 = st.tabs(["📊 Total Cost Comparison", "💊 Unit Dosage Cost (Price per Pill)", "🔍 Sensitivity Analysis"])
    ...
    with tab3:
        st.subheader("Sensitivity Analysis: Quantity and Strength Impact")
        
        st.info("""
        📊 **Sensitivity Analysis Methodology:** 
        This analysis shows how different Cost Plus formulation choices (quantity per package and drug strength) 
        affect total spending calculations. All prices include the $5 dispensing fee and 7.52% markup 
        (base_price + $5) × 1.0752 ÷ quantity_per_package. Four scenarios are analyzed for each drug:
        
        - **Low Quantity + Low Strength**: Smallest package size with lowest drug concentration
        - **Low Quantity + High Strength**: Smallest package size with highest drug concentration  
        - **High Quantity + Low Strength**: Largest package size with lowest drug concentration
        - **High Quantity + High Strength**: Largest package size with highest drug concentration
        """)
        
        # Filter sensitivity data based on selected drugs
        if selected_drugs:
            filtered_sensitivity_df = sensitivity_df[sensitivity_df['Cost Plus Drug'].isin(selected_drugs)].copy()
        else:
            filtered_sensitivity_df = sensitivity_df.copy()
        
        if filtered_sensitivity_df.empty:
            st.warning("No sensitivity analysis data available for selected drugs.")
        else:
            # Create summary by scenario
            st.write("**📊 Sensitivity Analysis Results**")
            
            # Summary metrics by scenario
            scenario_summary = filtered_sensitivity_df.groupby('Scenario').agg({
                'Total Spending ($)': ['mean', 'sum'],
                'Savings Amount ($)': ['mean', 'sum'], 
                'Savings Percentage (%)': 'mean'
            }).round(2)
            
            scenario_summary.columns = ['Avg Total Spending ($)', 'Total Spending ($)', 'Avg Savings ($)', 'Total Savings ($)', 'Avg Savings (%)']
            
            # Convert to millions for better display
            scenario_summary['Avg Total Spending ($M)'] = (scenario_summary['Avg Total Spending ($)'] / 1_000_000).round(2)
            scenario_summary['Total Spending ($M)'] = (scenario_summary['Total Spending ($)'] / 1_000_000).round(2) 
            scenario_summary['Avg Savings ($M)'] = (scenario_summary['Avg Savings ($)'] / 1_000_000).round(2)
            scenario_summary['Total Savings ($M)'] = (scenario_summary['Total Savings ($)'] / 1_000_000).round(2)
            
            # Display summary table
            display_summary = scenario_summary[['Avg Total Spending ($M)', 'Total Spending ($M)', 'Avg Savings ($M)', 'Total Savings ($M)', 'Avg Savings (%)']].copy()
            
            styled_scenario_summary = display_summary.style.format({
                'Avg Total Spending ($M)': '${:.2f}M',
                'Total Spending ($M)': '${:.2f}M', 
                'Avg Savings ($M)': '${:.2f}M',
                'Total Savings ($M)': '${:.2f}M',
                'Avg Savings (%)': '{:.1f}%'
            }).map(color_savings, subset=['Avg Savings ($M)', 'Total Savings ($M)', 'Avg Savings (%)'])
            
            st.dataframe(styled_scenario_summary, use_container_width=True)
            
            # Detailed drug-by-drug analysis
            st.write("**🔍 Detailed Drug-by-Drug Sensitivity Analysis**")
            
            # Create display dataframe with better formatting
            display_sensitivity_df = filtered_sensitivity_df[[
                'Medicare Generic Name', 'Cost Plus Drug', 'Scenario', 
                'Quantity', 'Strength (mg)', 'Base Price ($)', 'Unit Price with Fees ($)',
                'Formulation Details', 'Total Spending ($)', 'Medicare Total Spending ($)',
                'Savings Amount ($)', 'Savings Percentage (%)'
            ]].copy()
            
            # Convert spending columns to millions for better readability
            display_sensitivity_df['Total Spending ($M)'] = (display_sensitivity_df['Total Spending ($)'] / 1_000_000).round(3)
            display_sensitivity_df['Medicare Total Spending ($M)'] = (display_sensitivity_df['Medicare Total Spending ($)'] / 1_000_000).round(3)
            display_sensitivity_df['Savings Amount ($M)'] = (display_sensitivity_df['Savings Amount ($)'] / 1_000_000).round(3)
            
            # Rename columns for display
            display_sensitivity_df = display_sensitivity_df[[
                'Medicare Generic Name', 'Cost Plus Drug', 'Scenario', 
                'Quantity', 'Strength (mg)', 'Base Price ($)', 'Unit Price with Fees ($)',
                'Formulation Details', 'Total Spending ($M)', 'Medicare Total Spending ($M)',
                'Savings Amount ($M)', 'Savings Percentage (%)'
            ]].copy()
            
            display_sensitivity_df.columns = [
                'Drug Name', 'Cost Plus Drug', 'Scenario',
                'Qty/Pack', 'Strength (mg)', 'Base Price ($)', 'Unit Price w/Fees ($)',
                'Formulation', 'CP Total ($M)', 'Medicare Total ($M)', 
                'Savings ($M)', 'Savings (%)'
            ]
            
            # Style the dataframe
            styled_sensitivity_df = display_sensitivity_df.style.format({
                'Base Price ($)': '${:.2f}',
                'Unit Price w/Fees ($)': '${:.2f}',
                'CP Total ($M)': '${:.3f}M',
                'Medicare Total ($M)': '${:.3f}M',
                'Savings ($M)': '${:.3f}M',
                'Savings (%)': '{:.1f}%'
            }).map(color_savings, subset=['Savings ($M)', 'Savings (%)'])
            
            st.dataframe(styled_sensitivity_df, use_container_width=True, hide_index=True)
            
            # Visualization: Sensitivity by scenario
            st.subheader("📈 Sensitivity Visualization")
            
            # Create a chart showing total spending by scenario
            fig_sensitivity = px.box(
                filtered_sensitivity_df,
                x='Scenario',
                y='Savings Percentage (%)',
                title='Savings Distribution by Formulation Scenario',
                color='Scenario'
            )
            ...
            fig_sensitivity.update_layout(
                title=dict(font=dict(size=18, family="Arial Black")),
                xaxis=dict(
                    title=dict(text="Scenario", font=dict(size=16)),
                    tickfont=dict(size=14),
                    tickangle=45
                ),
                yaxis=dict(
                    title=dict(text="Savings Percentage (%)", font=dict(size=16)),
                    tickfont=dict(size=14),
                    tickformat='.1f',
                    ticksuffix="%"
                ),
            ...
            st.plotly_chart(fig_sensitivity, use_container_width=True, key="sensitivity_box_plot")
            
            # Summary insights
            st.subheader("💡 Key Insights")
            
            best_scenario = scenario_summary['Total Savings ($M)'].idxmax()
            worst_scenario = scenario_summary['Total Savings ($M)'].idxmin()
            best_savings = scenario_summary.loc[best_scenario, 'Total Savings ($M)']
            worst_savings = scenario_summary.loc[worst_scenario, 'Total Savings ($M)']
            ...
            with col1:
                st.metric(
                    "Best Scenario",
                    best_scenario,
                    f"${best_savings:.2f}M Total Savings"
                )
            ...
            with col2:
                st.metric(
                    "Worst Scenario", 
                    worst_scenario,
                    f"${worst_savings:.2f}M Total Savings"
                )
                
            with col3:
                sensitivity_range = best_savings - worst_savings
                st.metric(
                    "Sensitivity Range",
                    f"${sensitivity_range:.2f}M",
                    f"Difference between best/worst scenarios"
                )
