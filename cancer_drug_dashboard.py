...
def create_comparison_dataframe(matches_df: pd.DataFrame, costplus_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in matches_df.iterrows():
        ...
        if price_range['min_price'] is not None and price_range['max_price'] is not None:
            ...
            savings_vs_min = ((medicare_price - costplus_min) / medicare_price) * 100
            savings_vs_max = ((medicare_price - costplus_max) / medicare_price) * 100
            ...
            comparison_data.append({
                ...
                'Potential Savings (Max) %': savings_vs_min,
                'Potential Savings (Min) %': savings_vs_max
            })
...
def main():
    ...
    with tab1:
        ...
        total_spending_df = filtered_df[['Medicare Generic Name', 'Cost Plus Generic For',
                                       ...
                                       'Spending Reduction (%)', 'Potential Savings (Max) %', 'Potential Savings (Min) %']].copy()
        ...
        with st.expander("📋 Detailed Breakdown - Full Data", expanded=False):
            ...
            with col2:
                ...
                savings_detail_df = total_spending_df[['Medicare Generic Name', 'Cost Plus Generic For',
                                                     'Min Total Dollar Savings (Millions)', 'Max Total Dollar Savings (Millions)', 'Spending Reduction (%)',
                                                     'Potential Savings (Max) %', 'Potential Savings (Min) %']].copy()
                
                savings_detail_df.columns = ['Drug Name', 'CP Generic Name', 'Min Savings ($M)', 'Max Savings ($M)', 'Spending Reduction (%)', 'Max Savings (%)', 'Min Savings (%)']
                ...
                styled_savings_detail = savings_detail_df.style.format({
                    ...
                    'Min Savings (%)': format_percentage,
                    'Spending Reduction (%)': format_spending_reduction
                }).map(color_savings, subset=['Min Savings ($M)', 'Max Savings ($M)', 'Max Savings (%)', 'Min Savings (%)']).map(color_spending_reduction, subset=['Spending Reduction (%)'])
    ...
    with tab2:
        ...
        unit_summary_df.columns = ['Drug Name', 'Medicare Price', 'Cost Plus Max', 'Min Savings ($)', 'Min Savings (%)']
        ...
        styled_unit_summary = unit_summary_df.style.format({
            ...
            'Min Savings ($)': '${:.2f}',
            'Min Savings (%)': '{:.1f}%'
        }).map(color_savings, subset=['Min Savings ($)', 'Min Savings (%)'])
        ...
        with st.expander("📋 Detailed Breakdown - Full Unit Price Data", expanded=False):
            ...
            with col2:
                ...
                unit_savings_detail_df.columns = ['Drug Name', 'CP Generic Name', 'Min Savings ($)', 'Max Savings (%)', 'Min Savings (%)']
                
                styled_unit_savings_detail = unit_savings_detail_df.style.format({
                    'Min Savings ($)': '${:.2f}',
                    'Max Savings (%)': '{:.1f}%',
                    'Min Savings (%)': '{:.1f}%'
                }).map(color_savings, subset=['Min Savings ($)', 'Max Savings (%)', 'Min Savings (%)'])
